...
with st.sidebar:
    ...
    # BUG FIX (not in model answer): clamp slider max to actual region size.
    # Model answer uses hardcoded max=30; selecting East Asia (3 countries)
    # leaves slider at 15 with only 3 bars rendered — silent mismatch.
    max_n = len(df) if selected_region == 'All' else len(df[df['Region'] == selected_region])
    top_n = st.slider("Show top N countries", 1, max_n, min(15, max_n))
...
with col_left:
    ...
    # COLOUR TYPE: sequential — ordered bars, one direction (light→dark)
    # Double-encoding: position AND colour both convey rank → reinforces ordering
    fig1 = px.bar(
...
with col_right:
    ...
    # COLOUR TYPE: single colour — focus is on the pattern (correlation),
    # not on distinguishing groups
    fig2 = px.scatter(
...
# COLOUR TYPE: categorical — each factor is an unordered distinct category.
# CVD-safe: no red-green pair; blue-green-yellow-grey palette.
fig3 = px.bar(
